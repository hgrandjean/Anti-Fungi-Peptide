...
Employs linear regression to correlate the descriptor-based activity scores of the characterised AMPs with their IC50 and hydrophobic moment values. 

> **Linear regression** is a statistical method used to model the relationship between a dependent variable and one or more independent variables. It assumes a linear relationship between the variables and seeks to find the best-fit line that represents this relationship. 
> 
> Thus, the goal of linear regression is to estimate the coefficients of the line that minimize the difference between the observed data points and the predicted values.
